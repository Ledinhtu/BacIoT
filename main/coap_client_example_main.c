...
coap_start_pki_session(coap_context_t *ctx, coap_address_t *dst_addr, coap_uri_t *uri)
{
 ...
 static char client_sni[256];
    ...
    static coap_pdu_t *request = NULL;
    static unsigned char token[8];
    static size_t tokenlength;

static void coap_example_client_clean_up();
static esp_err_t coap_example_client_init(void *p);
static esp_err_t coap_example_client_send(char *str, uint8_t length);
...
static esp_err_t coap_example_client_send(char* str, uint8_t length) {
    // while (1) {
         ...
         *   coap_add_data_large_request(session, reques,t length, data, NULL, NULL);
         */

#ifdef POST_METHOD
        // u_char buf[12] = {'"','t','e','m','p','"',':','"','1','0','0','"'};
        // char* str = "{\"temp\":29}";
        ESP_LOGI(TAG, "String in function: %s", str);

        // uint8_t length = strlen(str);
        ESP_LOGI(TAG, "Length in function: %d", length);
        
        u_char *buf = NULL;
        buf = (u_char *)calloc(length, sizeof(u_char));
        if (buf == NULL) {
            ESP_LOGE(TAG, "calloc failed");
            return ESP_FAIL;
        }
        memcpy(buf, (u_char*)str, length);
        // ESP_LOGI(TAG, "Buf: %s", buf);
        ESP_LOGI(TAG, "Buf: %.*s", length, buf);

        // ESP_LOGI(TAG, "Before: %c", buf[0]);
        coap_insert_optlist(&optlist,
            coap_new_optlist(COAP_OPTION_CONTENT_FORMAT,
                            // coap_encode_var_safe (buf, sizeof (buf),
                            //             COAP_MEDIATYPE_APPLICATION_JSON),
        ...
        // ESP_LOGI(TAG, "After: %c", buf[0]);
        coap_add_data_large_request(session, request, length, buf, NULL, NULL);
#endif
        ...
        // ESP_LOGI(TAG, "After resp_wait");
        coap_send(session, request);
        // ESP_LOGI(TAG, "After coap_send");
...
static void coap_example_client(void *p)
{
    ...
    while (1)
    {
        if (dht_read_data(DHT_TYPE_DHT11, (gpio_num_t)DHT11_PIN, &humidity, &temperature) == ESP_OK)
        {
            ...
            // check_alarm();
            length = sprintf(str, "{\"ID\":%d,\n\"temp\":%d,\n\"humi\":%d}", 10, temperature/10, humidity/10);
            ESP_LOGI(TAG, "STR: %s", str);
            coap_example_client_send(str, length);
        }
        else
        {
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(coap_example_client_init(NULL));
    init_hw();

    xTaskCreate(coap_example_client, "coap", 10 * 1024, NULL, 5, NULL);
}
